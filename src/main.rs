...
async fn get_initial_config_and_services() -> anyhow::Result<(
    ...
    #[cfg(feature = "ee_feature")]
    {
        ...
        let pool = PgPool::connect(&database_url).await.map_err(|e| {
            anyhow::anyhow!(
                "Failed to connect to EE database at {}: {}",
                database_url,
                e
            )
        })?;
    ...
    #[cfg(not(feature = "ee_feature"))]
    {
        ...
        let yaml_config = config::load_config(&config_path).map_err(|e| {
            anyhow::anyhow!(
...
async fn main() -> anyhow::Result<()> {
    ...
    #[cfg(feature = "ee_feature")]
    {
        ...
        if let Some(ee_config_provider) = _ee_config_provider_opt {
            ...
            info!(
                "Starting DB configuration poller with interval: {:?}.",
                poll_duration
            );
